...
def get_gene_info(gff_file):
    ...
    suffix = gff_file.split(".")[-1].lower()

    if suffix not in ["gtf", "gff", "gff3"]:
        print("Input gene annotataion file is not in GFF/GFF3 format.\nPlease check the file.")
        sys.exit(1)
    ...
    with open(gff_file) as infile:
        for line in infile:
            ...
            chrom = info[0]
            category = info[2].lower()
            start = int(info[3])
            end = int(info[4])
            strand = info[6]
            desc = info[8].split(";")
            if category == "gene":
                gene_name = desc[0].split("=")[-1]
                genes_info[gene_name] = [chrom, start, end, strand]
            elif category == "transcript":
                gene_name = desc[0].split("\"")[1]
                genes_info[gene_name] = [chrom, start, end, strand]
            elif category == "cds":
                if gene_name in cds_info:
                    cds_info[gene_name] = [min(cds_info[gene_name][0], start), 
                                           max(cds_info[gene_name][1], end)]
                else:
                    cds_info[gene_name] = [start, end]
    
    for gene in genes_info:
        if gene in cds_info:
            s, e = cds_info[gene]
            dist1 = s - genes_info[gene][1]
            dist2 = genes_info[gene][2] - e
            if dist1 >= 0 and dist2 >= 0:
                genes_info[gene].extend([dist1, dist2])
            else:
                genes_info[gene].extend([0, 0])
        else:
            genes_info[gene].extend([0, 0])

    print("%s genes found in the annotation file.\n" % len(genes_info))
...
def generate_features(Features_info):
    ...
    if feature_file.endswith(".bw") or feature_file.endswith(".bigwig"):
        ...
        target_bed.intersect(feature_bed).moveto(outfile)
...
def main():
    ...
    withutr = int(config["General"]["withutr"])
    gff_file = config["Genes"]["gff_file"]
    chrom_sizes = config["Genes"]["chrom_sizes"]

    # Load genes
    if gff_file:
        genes_info = get_gene_info(gff_file)
        total_genes = len(genes_info)
    else:
        print("No genome annotation file found, stop!")
        sys.exit(1)
    ...
    for item in config["Features"]:
        ...
        for file in filelist:
            ...
            for gene in genes_info:
                ...
                utrst = genes_info[gene][4]
                utred = genes_info[gene][5]
                ...
                if strand == "+":
                    ...
                    if withutr:
                        feature_info.geneinfo.end = start + utrst
                    else:
                        feature_info.geneinfo.end = start
                else:
                    if withutr:
                        feature_info.geneinfo.start = end - utred
                    else:
                        feature_info.geneinfo.start = end
                    feature_info.geneinfo.end = end + upstream
            ...
            count += 1
            # continue                                        # Skip generate features raw data
            ...
            if file_suffix in ["bed", "bedgraph", "txt"] and filesize > 1e8:
                ...
                # Multiprocessing
                for i in range(0, total_genes, inputnum):
                    # Set Pool size
                    pool = Pool(threads)
                    if i + inputnum < total_genes:
                        inputlist = feature_infos[i:i+inputnum]
                    else:
                        inputlist = feature_infos[i:]
                    results = pool.map(generate_features, inputlist)
                    pool.close()
                    pool.join()
                    print("Round %s finished." % round(i/inputnum))
            time_ed = time()
    ...
    for i in range(0, total_genes, inputnum):
        ...
        if i + inputnum < total_genes:
            ...
            inputlist = feature_infos[i:]
        for result in pool.imap_unordered(run_analysis, inputlist):
            if result[1]:
                new += 1
            time_ed = time()
            if new:
                speed = round((time_ed - time_st) / new, 2)
            else:
                time_st = time()
                speed = 0.0
            print("%s / %s Gene (%s) analyzed (speed %s s)." % (cnt, total_genes, result[0], speed))
            cnt += 1
        pool.close()
        pool.join()
        print("Round %s finished." % round(i/inputnum))
